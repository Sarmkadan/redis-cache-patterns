...
public static class RedisCachePatternsOptionsExtensions
{
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="options"/> is null.</exception>
    /// <remarks>Uses invariant culture for consistent formatting.</remarks>
    ...
    /// </summary>
    /// <param name="options">The Redis cache options.</param>
    /// <returns>A redacted connection string suitable for logging; empty if the original is null or whitespace.</returns>
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="options"/> is null.</exception>
    public static string GetRedactedConnectionString(this RedisCachePatternsOptions options)
    {
        ...
        return string.Join(";", options.ConnectionString
            .Split(';', StringSplitOptions.RemoveEmptyEntries)
            .Select(part => part.StartsWith("password=", StringComparison.OrdinalIgnoreCase) && part.Length > 9
                ? "password=***"
                : part));
    }
}