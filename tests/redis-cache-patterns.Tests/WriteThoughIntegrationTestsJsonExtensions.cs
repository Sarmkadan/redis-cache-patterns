...
public static class WriteThoughIntegrationTestsJsonExtensions
{
	...
	/// <returns>The deserialized <see cref="WriteThoughIntegrationTests"/> instance, or <see langword="null"/> if the JSON represents a null value.</returns>
	...
	public static WriteThoughIntegrationTests? FromJson(string json)
	{
		...
		return JsonSerializer.Deserialize<WriteThoughIntegrationTests>(json, _jsonOptions);
	}
	...
	public static bool TryFromJson(string json, out WriteThoughIntegrationTests? value)
	{
		...
		try
		{
			value = JsonSerializer.Deserialize<WriteThoughIntegrationTests>(json, _jsonOptions);
			return true;
		}
		catch (JsonException)
		{
